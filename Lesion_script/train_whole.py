...
from Dataset_ import Pseudo_Pair_Whole_ImageDataset
...
parser.add_argument('--in_fold', type=str, default='./01_Whole_data/', help='input images path')
parser.add_argument('--normal', type=str, default='False', help='Train with normal data or not') 
...
parser.add_argument('--epoch', type=int, default=50, help='set epoch number') 
...
print('Data (Patients) : ',len(images_dir),len(labels_dir))
'''
...
# 病人如果有前一張沒有病灶的Normal圖片也可作為前一張作為參考(Optinal)
if(opt.normal == 'True'):
    normal_prior = glob.glob(opt.in_fold+'normal/*.png')
    normal_prior.sort()
    normal_prior_next = ['BS00016_20170606_3627', 'BS00022_20160322_9d26', 'BS00029_20120522_110f', 'BS00035_20161011_449b', 'BS00037_20171227_5224', 'BS00039_20200717_3c08', 
                         'BS00062_20140701_bb51', 'BS00071_20141014_3098', 'BS00080_20160425_50b9', 'BS00110_20191224_fbb7', 'BS00111_20140210_1e2f', 'BS00130_20190827_c506', 
                         'BS00134_20171031_f36b', 'BS00136_20150929_91f7', 'BS00146_20140320_fef6', 'BS00153_20151109_429d', 'BS00163_20190723_8b44', 'BS00192_20200722_a188', 
                         'BS00201_20161216_51bc', 'BS00208_20180807_ca1b',]
...
if(opt.train=='True'):
    ...
    for i in range(1,len(train)):
        file_1 = train[i-1][0][-26:-18] 
        file_2 = train[i][0][-26:-18] 
        image_t = []
        if(file_1 == file_2):
        ...
        elif(opt.normal == 'True' and train[i][0][-25:-4] in normal_prior_next):
            index = normal_prior_next.index(train[i][0][-25:-4])
            image_t.append(train[i][0])
            image_t.append(normal_prior[index])
            train_pair_images.append(image_t)
            train_pair_labels.append(train[i][1])
            find +=1
        else:
...
for i in range(1,len(valid)):
    file_1 = valid[i-1][0][-26:-18]
    file_2 = valid[i][0][-26:-18] 
    image_t = []
    if(file_1 == file_2):
    ...
    elif(opt.normal == 'True' and valid[i][0][-25:-4] in normal_prior_next):
        index = normal_prior_next.index(valid[i][0][-25:-4])
        image_t.append(valid[i][0])
        image_t.append(normal_prior[index])
        valid_pair_images.append(image_t)
        valid_pair_labels.append(valid[i][1])
...
if(opt.pseudo == 'True'):
    ...
    for i in range(1,len(pseudo_train)):
        file_1 = pseudo_train[i-1][0][-26:-18] 
        file_2 = pseudo_train[i][0][-26:-18] 
        image_t = []
        if(file_1 == file_2):
            pair+=1
...
if(opt.train=='True'):
    train_dataset = Pair_Whole_ImageDataset(pair_train, train_transform, train2_transform)
    print("Dataset (Train) : ",len(train_dataset))

valid_dataset = Pair_Whole_ImageDataset(pair_valid, valid_transform, valid_transform)
print("Dataset (Valid) : ", len(valid_dataset))


if(opt.pseudo == 'True'):
    pseudo_train_dataset = Pseudo_Pair_Whole_ImageDataset(pseudo_pair_train, train_transform, train2_transform)
    print("Psuedo dataset :", len(pseudo_train_dataset))
...
save_best_path = opt.out_weight + Name + '_whole_best'
save_path = opt.out_weight + Name + '_whole'
