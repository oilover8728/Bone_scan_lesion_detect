...
from Dataset_ import Pseudo_Pair_Registration_ImageDataset
...
parser.add_argument('--in_fold', type=str, default='./03_Patch_registration/', help='input images path') 
...
valid_pair_labels = []
images_count = 0
last_name_p = valid_labels[0][-32:-22]
temp_set = set()
for i in range(len(valid_labels)):
    image_t = []
    name_position = valid_labels[i][-32:-22]
    if(valid_labels[i][-20:-19]=='1'):
        image_t.append(valid_images[images_count])
        image_t.append(valid_images[images_count+1])
        valid_pair_images.append(image_t)
        valid_pair_labels.append(valid_labels[i])
        images_count+=2
    else:
        image_t.append(valid_images[images_count])
        image_t.append(valid_images[images_count])
...
if(opt.pseudo == 'True'):
    ...
    for i in range(len(pseudo_labels)):
        image_t = []
    #     print(valid_labels[i][-20:-19])
        if(pseudo_labels[i][-20:-19]=='1'):
            image_t.append(pseudo_images[images_count])
            image_t.append(pseudo_images[images_count+1])
            pseudo_pair_images.append(image_t)
            pseudo_pair_labels.append(pseudo_labels[i])
            images_count+=2
        else:
            image_t.append(pseudo_images[images_count])
            image_t.append(pseudo_images[images_count])
    ...
    pseudo_pair_train=[]
    for i in range(len(pseudo_pair_images)):
        package=[]
        package.append(pseudo_pair_images[i])
        package.append(pseudo_pair_labels[i])
        pseudo_pair_train.append(package)

    print('Pseudo data : ',len(pseudo_pair_train))
